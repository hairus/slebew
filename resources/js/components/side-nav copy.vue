...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 */
			// async getUser(){
			//     await this.user = localStorage.getItem('user');
			// },
			hasItems(item) {
	...
	<div id="sidebar-menu">
		...
		<ul
			...
			<template v-for="item in menuItems" :key="item.id">
				<!-- role admin -->
				<div v-if="user.roles[0]['role'] === 'admin'">
					...
					<li
						v-if="
							!item.isTitle && !item.isLayout && item.role === user.roles[0]['role']
						"
						:key="item.id">
						<BLink
							v-if="hasItems(item)"
				...
				<!-- role guru -->
				<div v-if="user.roles[0]['role'] === 'guru'">
					<div v-if="item.role === user.roles[0]['role']">
						<li
							class="menu-title"
							v-if="item.isTitle"
