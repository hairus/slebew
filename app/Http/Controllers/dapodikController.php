...
class dapodikController extends Controller
{
  ...
  public function mapels()
  {
    ...
    $pem = pembelajaran::get();

    return $pem;
  }

  public function simpanMapels()
  {
    $pem = pembelajaran::get();

    foreach ($pem as $data) {
      $cek = mapels::where('mata_pelajaran_id', $data->mata_pelajaran_id)->first();
      if ($cek) {
        //update
        $cek->update([
          'pembelajaran_id' => $data->pembelajaran_id,
          'mata_pelajaran_id' => $data->mata_pelajaran_id,
          'mata_pelajaran_id_str' => $data->mata_pelajaran_id_str,
        ]);
      } else {
        //insert
        mapels::create([
    ...
    foreach ($ptks as $data) {
      ...
      if ($cek) {
        ...
        $cek->update([
          ...
          'nama' => $data->nama,
          'jenis_ptk_id_str' => $data->jenis_ptk_id_str,
        ]);
      } else {
        // create
        guru::create([
  ...
  public function simKelas()
  {
    $rombels = rombongan_belajar::get();
    foreach ($rombels as $data) {
      $cek = kelas::where('rombongan_belajar_id', $data->id)->first();
      if (!$cek) {
        // create
        kelas::create([
  ...
  public function singkron()
  {
    ...
    $rombels = rombongan_belajar::get();
    foreach ($rombels as $data) {
      $cek = kelas::where('rombongan_belajar_id', $data->rombongan_belajar_id)->first();
      if (!$cek) {
        // create
        kelas::create([
          "rombongan_belajar_id" => $data->rombongan_belajar_id,
