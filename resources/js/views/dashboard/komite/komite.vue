						...
						<BForm>
							<BFormGroup
								...
								<BFormInput
									...
									type="number"
									v-model="form.price"
									placeholder="Enter Your Price"></BFormInput>
							</BFormGroup>
							<div>
								<BButton
									type="submit"
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getPrice() {
				await axios.get('/api/admin/getPrice').then((res) => {
			...
			async removePrice(id) {
				this.loading = true;
				await axios.delete('/api/admin/delPrice/' + id).then((res) => {
					this.getPrice();
					this.loading = false;
				});
			},
